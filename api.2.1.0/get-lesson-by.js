/**
 * API: /1.0.0/instances/prod/lessons/get-lessons-by 
 * METHOD : GET
 * PARAMETERS: count, page, keyword, sessionid
 * EX: /1.0.0/instances/prod/lessons/get-lessons-by?count=1&keyword=Welcome%20to%20ch&page=2&sessionid=ea63379bebe07f765f1125d791af4e17b01d1c552b6ee7eb9b4e1354c391285a244f
 * 
**/
...
exports.fn = async function(req, res) {
  ...
  try{
    ...
    // fetch lessons based on keyword, count and page
    // let lessons = await getQuery("Select v3_id, hash_code, type, title, level, image, video, introduction, mp3_dialogue, mp3_thefix, lesson_plan, mp3_private as high_quality_mp3, mp3_public as radio_quality_mp3 from contents where title like '"+keyword+"%'")
    ...
    let lessons = await m158.db("chinesepod").collection("lessons").find (query).skip((page-1) * count).limit(count).project(projection).toArray ();
    let freeLessons = await m158.db("chinesepod").collection("legacy.cp.prod.contents_free").find (query).project(projection).toArray ();

    // traverse and format values
    for (let l=0; l < lessons.length; l++) {
      ...
      lessons[l].introduction = res.helpers.sanitizeHTML(lessons[l].introduction);
